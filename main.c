...
void playmove(bool* ctp, struct move playedMove, struct piece wPieces[16], struct piece bPieces[16]){
    ...
    if (playedMove.capture == -1) {
        if (*ctp) {
            for (int i = 0; i < 16; i++) {
                if (bPieces[i].rank == playedMove.targetRank && bPieces[i].file == playedMove.targetFile) {
                    bPieces[i].alive = 0;
                }
            }
        }
            ...
            for (int i = 0; i < 16; i++) {
                if (wPieces[i].rank == playedMove.targetRank && wPieces[i].file == playedMove.targetFile) {
                    wPieces[i].alive = 0;
                }
            }
        }
